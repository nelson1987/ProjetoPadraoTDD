...
namespace WebForDocs.Biblioteca
{
    public static class ThreadRoboGovernanca
    {
        ...
        private static WFD_SOLICITACAO BuscarsolicitacaoParaRobo(WFLModel db, int id)
        {
        ...
        static void ChamaRobo(int solicitacaoId)
        {
            try
            {
                ...
                var robo = solicitacao.WFD_PJPF_ROBO.FirstOrDefault();
        ...
        private static void GravaLog(WFLModel db, int? Code, string Mensagem, string orgao, int contratanteId, int? pjpfId, int? solicitacaoId)
        {
