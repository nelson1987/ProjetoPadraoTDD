    ...
    public class RoboSintegra
    {
        ...
        public RoboSintegra CarregaSintegra(string estado, string cnpj, string path)
        {
            ...
            try
            {
                ...
                {
                    var ativarConsultasRobo = Convert.ToBoolean(ConfigurationManager.AppSettings["AtivarConsultasRobo"]);
                    string textResult = string.Empty;

                    if (!ativarConsultasRobo)
        ...
        public static RoboSintegra ModelToViewModel(ROBO robo)
        {
        ...
        private void MontaCssMessagem()
        {
            if (Code == 1)
            {
                 cssCor = ( Data.SituacaoCadastral.ToUpper() == "HABILITADO ATIVO" ||  Data.SituacaoCadastral.ToUpper() == "HABILITADO" ? "success" : "warning");
                 Data.Message = Code + " - Consulta Realizada com sucesso. (Situação Cadastral: " + Data.SituacaoCadastral + ") &nbsp;&nbsp;&nbsp;<i class='fa fa-arrow-circle-down'></i>";
            }
            else if (Code == 2)
            {
                cssCor = "success";
                Data.Message = Code + " - ISENTO";
            }
            else
            {
                cssCor = "danger";
                Data.Message = Code + " - " + Data.Message;
            }
        }
    }
